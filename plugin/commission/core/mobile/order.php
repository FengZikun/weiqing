...
class Order_EweiShopV2Page extends CommissionMobileLoginPage {
	...
	public function agentpay() 
	{
		...
		if (1 < $order['status']) 
		{
			$this->message('订单已付款，不需重复付款！', '', 'error');
		}
		
		if (!(empty($order['virtual'])) && com('virtual')) 
		{
			com('virtual')->pay($order);
		}
		else 
		{
			$result = pdo_update('ewei_shop_order', array('status' => 1, 'paytype' => 11, 'paytime' => time()), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
			if (!empty($result)) {
				...
				$this->message('订单确认付款成功！', mobileUrl('commission/order'), 'success');
			}
		}
	...
	public function agentsend() 
	{
		...
		$time = time();
		$data = array('status' => 2, 'sendtype' => (0 < $order['sendtype'] ? $order['sendtype'] : intval($_GPC['sendtype'])), 'express' => trim($_GPC['express']), 'expresscom' => trim($_GPC['expresscom']), 'expresssn' => trim($_GPC['expresssn']), 'sendtime' => $time);
			
		$result = pdo_update('ewei_shop_order', $data, array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
		...
		if (!empty($result)) {	
			...
			if (!(empty($order['refundid']))) 
			{
				...
				if (!(empty($refund))) 
				{
					...
					pdo_update('ewei_shop_order', array('refundstate' => 0), array('id' => $order['id']));
				}
			}
			...
			$this->message('订单发货成功！', mobileUrl('commission/order'), 'success');
