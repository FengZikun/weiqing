			...
			if ($mobile && !(empty($page['data']['items'])) && is_array($page['data']['items'])) 
			{
				foreach ($page['data']['items'] as $itemid => &$item ) 
				{
					...
					else if ($item['id'] == 'member') 
					{
						...
						$params[':y1'] = $begin;
						$params[':y2'] = $end;
						$member['credit10yesterday'] = pdo_fetchcolumn("select priceevery from ". tablename('ewei_shop_ydfullback_log') ." where createtime>=:y1 and createtime <:y2 and openid=:openid and uniacid=:uniacid  limit 1",$params);
						$params = array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']);
						$member['credit10total'] = pdo_fetchcolumn("select sum(priceevery) from ". tablename('ewei_shop_ydfullback_log') ." where openid=:openid and uniacid=:uniacid  limit 1",$params);
						$member['credit10totalconsume'] = pdo_fetchcolumn("select sum(deduction)+sum(money) from ". tablename('ewei_shop_cashier_pay_log') ." where openid=:openid and uniacid=:uniacid and status =1  limit 1",$params);
						$member['credit10totalconsume'] =number_format($member['credit10totalconsume']/0.12 ,2);
						...
						, 'credit11' => $member['credit11']
						,'credit10yesterday' =>$member['credit10yesterday']
						,'credit10totalconsume' =>$member['credit10totalconsume']
						,'credit10total'=>$member['credit10total']);
					}
