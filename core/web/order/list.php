...
class List_EweiShopV2Page extends WebPage 
{
	protected function orderData($status, $st) 
	{
		...
		if (!(empty($_GPC['searchfield'])) && !(empty($_GPC['keyword']))) 
		{
			...
			else if ($searchfield == 'level') 
			{
				$condition .= ' AND locate(:keyword,o.levelname)>0';
			}
		...
		$paytype = array( 0 => array('css' => 'default', 'name' => '未支付'), 1 => array('css' => 'danger', 'name' => '余额支付'), 11 => array('css' => 'default', 'name' => '后台付款'), 2 => array('css' => 'danger', 'name' => '在线支付'), 21 => array('css' => 'success', 'name' => '微信支付'), 22 => array('css' => 'warning', 'name' => '支付宝支付'), 23 => array('css' => 'warning', 'name' => '银联支付'),33 => array('css' => 'warning', 'name' => '益豆支付'), 3 => array('css' => 'primary', 'name' => '货到付款') );
