...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
			...
			if (!(empty($saleset))) 
			{
				...
				if (!(empty($saleset['credityddeduct'])))
				{
					$pcredit = intval($saleset['credit']);
					$pmoney = round(floatval($saleset['ydmoney']), 2);
					if ((0 < $pcredit) && (0 < $pmoney))
					{
						$yddeductmoney = round($credit10 / $pcredit * $pmoney, 2);
						$yddeductmoney_source = $yddeductmoney;
					}
					if ($realprice < $yddeductmoney)
					{
						$yddeductmoney = $realprice;
					}
					$yddeductcredit = floor((($pmoney * $pcredit) == 0 ? 0 : ($yddeductmoney / $pmoney) * $pcredit));
				}
	...
	public function caculate() 
	{
		...
		if (is_array($goodsarr)) 
		{
			...
			$ydorderrate = $data['ydorderrate']/100;
			...
			if (!(empty($saleset))) 
			{
				...
				if (!(empty($saleset['credityddeduct'])))
				{
					...
					if ($realprice*$ydorderrate < $yddeductmoney)
					{
						$yddeductmoney = $realprice*$ydorderrate;
					}
		...
		$return_array['yddeductcredit'] = $yddeductcredit;
		$return_array['yddeductmoney'] = $yddeductmoney;
	...
	public function submit() 
	{
		...
		if (empty($goods[0]['bargain_id'])) 
		{
			...
			if (!(empty($saleset['credityddeduct'])))
			{
				if (!(empty($_GPC['deduct3'])))
				{
					$credit10 = $member['credit10'];
					$pcredit = intval($saleset['credit']);
					$pmoney = round(floatval($saleset['ydmoney']), 2);
					$data3 = m('common')->getSysset('shop');
					$ydorderrate = $data3['ydorderrate']/100;
					if ((0 < $pcredit) && (0 < $pmoney))
					{
						$yddeductmoney = round($credit10 / $pcredit * $pmoney, 2);
					}

					if($yddeductmoney>$order_total_price*$ydorderrate){
						$totalprice = $totalprice-$order_total_price*($ydorderrate);
						$deductcredit3 = round($order_total_price*$ydorderrate/$pmoney,2);
					}
					else{
						$deductcredit3 = $credit10;
						$totalprice -=$yddeductmoney;
					}


//					if($yddeductmoney>$totalprice){
//						$deductcredit_10 = round($totalprice/$pmoney,2);
//						$deductcredit3 = $deductcredit_10;
//						$totalprice =0;
//					}else{
//						$deductcredit_10 = $credit10;
//						$deductcredit3 = $deductcredit_10;
//						$totalprice -=$yddeductmoney;
//					}
				}
		...
		$order['deductcredit3'] = $deductcredit3;
		...
		if (0 < $deductcredit3)
		{
			m('member')->setCredit($openid, 'credit10', -$deductcredit3, array('0', $_W['shopset']['shop']['name'] . '购物抵扣抵扣: ' . $deductcredit3 . ' 订单号: ' . $ordersn));
		}
		if (empty($virtualid)) 
		{
