...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		if (empty($order['price'])) 
		{
			...
			header('location: ' . mobileUrl('order/pay/complete', array('id' => $order['id'], 'type' => $order['deductcredit3']?'ydpay':'credit')));
			exit();
		}
		load()->model('payment');
	...
	public function complete() 
	{
		...
		$type = $_GPC['type'];
		if (!(in_array($type, array('wechat', 'alipay', 'credit', 'cash','ydpay'))))
		{
			if ($_W['ispost']) 
			{
				show_json(0, '未找到支付方式');
		...
		}if ($type == 'ydpay')
		{
			...
			$record['type'] = 'ydpay';
			pdo_update('core_paylog', $record, array('plid' => $log['plid']));
			m('order')->setOrderPayType($order['id'], 33);
			$ret = array();
			$ret['result'] = 'success';
			$ret['type'] = $log['type'];
			$ret['from'] = 'return';
